 ...
 */
export function changeCase(str: string, caseType: 'camel' | 'snake' | 'kebab' | 'pascal' | 'title' | 'sentence' | 'lower' | 'upper' | 'constant' | 'path' | 'none'): string {
    if (!str) return '';
 ...
 */
export function humanizeString(input: string, capitalizeFirstLetter: boolean = true, capitalizeAllWords: boolean = false): string {
    if (!input) return '';
    ...
    if (capitalizeAllWords) str = str.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    return str;
}
 ...
 */
export function replaceStringTokens(str: string, replacements: { [key: string]: any }, identifier: string = ":"): string {
    if (!str) return '';
