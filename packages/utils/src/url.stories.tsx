      ...
      <div className="space-y-4">
        {examples.map((example, index) => {
          ...
          const passed =
            ...
            (error === null &&
              JSON.stringify(result) === JSON.stringify(example.expected));
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h3 className="text-xl font-bold mb-4">
        🧪 Interactive URL Function Tester
      </h3>
      ...
      {input && (
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm font-mono">
            ...
            <div>
              <strong>getDomainName:</strong>{" "}
              {JSON.stringify(getDomainName(input))}
            </div>
            <div>
              <strong>replaceDoubleSlashes:</strong> "
              {replaceDoubleSlashes(input)}"
            </div>
            <div>
              <strong>getSegment (/, 1):</strong>{" "}
              {JSON.stringify(getSegment(input, "/", 1))}
            </div>
            <div>
              <strong>getSegment (/, 2):</strong>{" "}
              {JSON.stringify(getSegment(input, "/", 2))}
            </div>
          </div>
        </div>
      )}
...
const meta: Meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "Testing and documentation for URL utility functions from @ottabase/utils",
      },
    },
  },
};
...
export const InteractiveTest: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Interactive tester for URL utility functions. Enter text and see results in real-time.",
      },
    },
  },
};
...
export const MakeSlugTests: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Test cases for the makeSlug function showing input, output, and expected results.",
      },
    },
  },
};
...
export const GetDomainNameTests: Story = {
  render: () => (
    <FunctionDemo
      title="getDomainName()"
      fn={getDomainName}
      examples={getDomainNameExamples}
    />
  ),
  parameters: {
    docs: {
