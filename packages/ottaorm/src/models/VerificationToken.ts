...
export type NewVerificationTokenType =
...
export class VerificationToken extends BaseModel {
   ...
   */
  static async findByIdentifierAndToken(
    identifier: string,
    token: string,
    ...
    const result = await db
      .prepare(
        `SELECT * FROM ${this.entity} WHERE identifier = ? AND token = ?`,
      )
   ...
   */
  static async deleteByIdentifierAndToken(
    identifier: string,
    token: string,
  ): Promise<boolean> {
    const driver = this.getDriver();
    ...
    const result = await db.run(
      `DELETE FROM ${this.entity} WHERE identifier = ? AND token = ?`,
      identifier,
      token,
    );
