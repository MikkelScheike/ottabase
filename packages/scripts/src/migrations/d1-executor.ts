...
function parseWranglerToml(
  ...
  try {
    ...
    return {
      valid: false,
      error: `D1 binding "${database}" not found in ${path.basename(
        configPath,
      )}`,
    };
  } catch (error) {
 ...
 */
export function validateD1Binding(
  database: string,
  cwd: string,
  wranglerConfig?: string,
): D1ValidationResult {
  ...
  if (!configPath) {
    return {
      valid: false,
      error:
        "No wrangler configuration file found. Create wrangler.toml or wrangler.jsonc",
    };
  }
  ...
  if (existsSync(migrationPath)) {
    ...
    if (stats.isDirectory()) {
      ...
      if (!existsSync(sqlFilePath)) {
        throw new Error(
          `No migration.sql found in directory: ${migrationPath}`,
        );
      }
    } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `Migration execution failed (exit code ${code}):\n${errorMsg}`,
          ),
        );
      }
    });
