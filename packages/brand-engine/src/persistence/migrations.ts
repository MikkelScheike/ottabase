...
export const brandEngineMigrations: Migration[] = [
    {
        ...
        up: async (db) => {
            await db.execute(`CREATE INDEX IF NOT EXISTS menu_items_appid_idx ON menu_items(app_id)`);
            await db.execute(`CREATE INDEX IF NOT EXISTS menu_items_menuid_idx ON menu_items(menu_id)`);
        },
    },
];
