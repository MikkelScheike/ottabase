...
export default function BlogPagination({ page = 1, lastPage = 1, perPage = 30, isLoading, showNextPrev = true, showPageNumbers = true, onPageChange }: BlogPaginationProps) {
        ...
        <nav className="flex items-center justify-between border-t border-gray-200 dark:border-gray-700 px-4 sm:px-0">
            ...
            <div className="hidden md:-mt-px md:flex">
                {showPageNumbers && getPageNumbers().map((pageNum, index) => (
                    pageNum === '...' ? (
                        <span key={`ellipsis-${index}`} className="inline-flex items-center border-t-2 border-transparent px-4 pt-4 text-sm font-medium text-gray-500 dark:text-gray-400">
                            ...
                        </span>
                    ) : (
                        <button
                            ...
                            disabled={isLoading}
                            className={`inline-flex items-center border-t-2 px-4 pt-4 text-sm font-medium ${pageNum === page
