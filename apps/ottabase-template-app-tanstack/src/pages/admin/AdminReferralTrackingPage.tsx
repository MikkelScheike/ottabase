...
export function AdminReferralTrackingPage() {
  ...
  const { data: recentTracking, isLoading: trackingLoading } = useQuery<
    ReferralTrackingData[]
  >({
    queryKey: ["referral-tracking-recent"],
    queryFn: async () => {
      const response = await api<ReferralTrackingData[]>(
        "/api/referrals/tracking/recent?limit=20"
      );
      return response;
    },
  });
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Latest 20 referral clicks and conversions
          </CardDescription>
        </CardHeader>
        <CardContent>
          {trackingLoading ? (
